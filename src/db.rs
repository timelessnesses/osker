...
pub async fn connect_to_db() -> Result<sea_orm::DatabaseConnection, sea_orm::DbErr> {
    dotenv::dotenv().ok();
    let db_type = match std::env::var("OSKER_DB_TYPE").expect("OSKER_DB_TYPE is not found").as_str() {
    ...
    if db_type == "sqlite" {
        ...
        let port = std::env::var("OSKER_DB_PORT").expect("OSKER_DB_PORT").parse::<u16>().expect("Failed to parse integer");
        let database = std::env::var("OSKER_DB").expect("OSKER_DB is not found");
        sea_orm::Database::connect(format!("{db_type}://{username}:{password}@{host}:{port}/{database}")).await
    }
}